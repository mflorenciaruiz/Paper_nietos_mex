...
corr viv_emig_10 viv_emig_00 spanish_presence spanish_presence_1936_1955 spanish_presence_1956_1978
	...
	* ALTERNATIVA 1: solo remesas
ebalance spanish_presence remesas_pre_avg, targets(1) gen(w2_1)

	* ALTERNATIVA 2: parsimoniosa
ebalance spanish_presence log_remesas_pre_avg viv_emig_10 im_2010  log_poblacion_2010 log_densidad, targets(2) gen(w2_2)

	* ALTERNATIVA 3: intermedia con temporalidad 
ebalance spanish_presence log_remesas_pre_avg viv_emig_00 viv_emig_10 im_2000 im_2010 log_poblacion_2010 log_densidad pea_pct, targets(2) gen(w2_3)

	* ALTERNATIVA 4: capital humano
ebalance spanish_presence log_remesas_pre_avg viv_emig_00 viv_emig_10 im_2010 log_poblacion_2010 log_densidad ///
	pea_pct sin_escolaridad_pct esc_basica_pct, targets(2) gen(w2_4)

/// si uso remesas_pre_avg solo balancea la media y sin usar covariables extra

* Análisis de las alternativas
	* Distribución de los pesos
histogram w2_1, name(g1, replace) title("w 2.1") xtitle(Entropy balancing weights)
histogram w2_2, name(g2, replace) title("w 2.2") xtitle(Entropy balancing weights)
histogram w2_3, name(g3, replace) title("w 2.3") xtitle(Entropy balancing weights)
histogram w2_4, name(g4, replace) title("w 2.4") xtitle(Entropy balancing weights)
graph combine g1 g2 g3 g4, col(2)
graph export "$out_folder/weights_panels_12.png", replace

	* Estadísticas
estpost tabstat w1_1 w1_2 w1_3 w1_4, stats(mean min max v p1 p5 p10 p50 p90 p95 p99) columns(statistics)
esttab using "$out_folder/weights_summary_11.csv", ///
    cells("mean min max variance p1 p5 p10 p50 p90 p95 p99") ///
    noobs nonumber nomtitle nonote plain replace
...
esttab using "$out_folder/balance_summary_11.csv", ///
	...
	* --- Alternativas sin outcome pre (con pesos máximos menores a 0.5 en 1.1): 5 6 8 10 11 12--- *
	
	* ALTERNATIVA 5: marginalización 2010, densidad
ebalance spanish_presence im_2010 log_densidad, targets(1) gen(w11_5)

	* ALTERNATIVA 6: marginalización 2010, densidad, migración 2010
ebalance spanish_presence viv_emig_10 im_2010 log_densidad, targets(1) gen(w11_6)
	
	* ALTERNATIVA 8: marginalización 2010, densidad, migración 2010, pea
ebalance spanish_presence viv_emig_10 im_2010 log_densidad pea_pct, targets(1) gen(w11_8)
	
	* ALTERNATIVA 10: marginalización 2010, densidad, migración 2010, pea, escolaridad media superior
ebalance spanish_presence viv_emig_10 im_2010 log_densidad pea_pct esc_media_superior_pct, targets(1) gen(w11_10)

	* ALTERNATIVA 11: marginalización 2010, densidad, migración 2010, pea, escolaridad básica
ebalance spanish_presence viv_emig_10 im_2010 log_densidad pea_pct esc_basica_pct, targets(1) gen(w11_11)

	* ALTERNATIVA 12: marginalización 2010, densidad, migración 2010, pea, sin escolaridad
ebalance spanish_presence viv_emig_10 im_2010 log_densidad pea_pct sin_escolaridad_pct, targets(1) gen(w11_12)

}

*** 1.3) Balanceno presencia española por ventanas (4 grupos)
{
...
*** 1.4) Balanceno presencia española por ventanas (3 grupos)
{
